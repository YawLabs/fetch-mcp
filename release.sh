...
if [ "$PUBLISHED_VERSION" = "$VERSION" ]; then
  ...
  info "Published @yawlabs/fetch-mcp@${VERSION} to npm (with provenance)"
...
if [ ! -f server.json ]; then
  ...
  # live on npm, so we confirm that a fresh install via npx can execute the
  # binary and respond to --version. Catches packaging regressions (missing
  # bin shebang, bad "files" entry, broken tsup output) before they hit real
  # users. Skipped in CI mode here -- CI runs its own smoke and this is the
  # local-publish path.
  if [ "$IS_CI" != "true" ]; then
    # Run npx from a temp dir -- if run from the checkout root, npx sees our
    # own package.json `bin` entry and tries to resolve the local (unbuilt)
    # path instead of installing the published tarball.
    SMOKE_DIR=$(mktemp -d)
    # Registry propagation can lag well past a minute after publish succeeds,
    # and `npm view` and `npx` may hit different CDN paths. Retry the npx
    # invocation itself with a budget generous enough to outlast realistic
    # propagation. 30 * 10s = ~5min upper bound; typical case completes in
    # < 30s.
    ATTEMPTS=30
    SLEEP_SECONDS=10
    SMOKE_OUTPUT=""
    ...
    for i in $(seq 1 $ATTEMPTS); do
      if SMOKE_OUTPUT=$(cd "$SMOKE_DIR" && npx -y "@yawlabs/fetch-mcp@${VERSION}" --version 2>/dev/null); then
        info "npx smoke: $SMOKE_OUTPUT (after $(( $(date +%s) - SMOKE_STARTED_AT ))s)"
        break
      fi
      warn "Waiting for @yawlabs/fetch-mcp@${VERSION} to be installable via npx (attempt $i/$ATTEMPTS, ${SLEEP_SECONDS}s)..."
      sleep $SLEEP_SECONDS
    done
    rm -rf "$SMOKE_DIR"
    if [ "$SMOKE_OUTPUT" != "$VERSION" ]; then
      fail "Smoke test expected $VERSION, got '$SMOKE_OUTPUT' after $ATTEMPTS attempts ($(( $(date +%s) - SMOKE_STARTED_AT ))s)"
    fi
  fi
