...
8. **Request timeout and cancellation via `AbortController`.** `timeout_ms` bounds each attempt end to end -- DNS, every redirect hop, headers and body -- defaulting to 10s and capped at 120s by the tool schema. Retries get a fresh budget, and a whole call is capped at 5 minutes. Cancelling the MCP tool call aborts the hop in flight (DNS included), any Retry-After wait and every later attempt.
