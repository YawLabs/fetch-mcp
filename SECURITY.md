...
2. **IP literal check.** URLs whose host is a literal IP are rejected if the IP falls into a loopback, RFC1918 private, link-local (incl. 169.254.169.254 cloud metadata), CGNAT, ULA, multicast, or reserved range. IPv4-mapped IPv6 literals are re-checked against the IPv4 rules, as is the IPv4 embedded in IPv4-compatible (`::/96`), IPv4-translated (`::ffff:0:0:0/96`) and 6to4 (`2002::/16`) addresses; Teredo (`2001::/32`), NAT64 (`64:ff9b::/96`, `64:ff9b:1::/48`), site-local (`fec0::/10`) and discard-only (`100::/64`) are refused outright, and so is the Azure WireServer address `168.63.129.16`. Beyond those, IPv6 is an allow-list: only global unicast (`2000::/3`) passes, minus `3fff::/20`, `2001:2::/48` and `2001:10::/28`.
3. **`localhost*` hostname check.** The names `localhost` and `*.localhost` are refused even though DNS could map them anywhere.
4. **DNS pre-resolve with IP pinning.** For every non-literal host we resolve via `dns.lookup` and check every returned address. If any address is in a blocked range, the request is refused -- with a message that does not name the resolved address, so a refusal cannot be used to map internal hostnames to IPs. The IP we verified is then pinned into an undici dispatcher so the subsequent TCP connection dials THAT address -- not a different one returned by a concurrent DNS query. This closes the classic DNS-rebinding TOCTOU window.
...
8. **Request timeout and cancellation via `AbortController`.** `timeout_ms` bounds each attempt end to end -- DNS, every redirect hop, headers and body -- defaulting to 10s and capped at 120s by the tool schema. Retries get a fresh budget, and a whole call is capped at 5 minutes -- a `fetch_sitemap` call too, across every document it fetches. A connect in progress is abandoned the moment the deadline or a cancellation fires. Cancelling the MCP tool call aborts the hop in flight (DNS included), any Retry-After wait and every later attempt.
...
11. **Operator gate on `allow_private_hosts`.** The per-call `allow_private_hosts: true` opt-in is a tool argument, and the LLM chooses tool arguments -- so on its own it would hand the injected prompt above a one-flag bypass. It is refused (before any DNS or network work) unless the operator launched the server with `FETCH_MCP_ALLOW_PRIVATE_HOSTS=1`. The value is parsed strictly -- `1`/`true`/`yes`/`on`, anything unrecognised is OFF and named on stderr -- and under the oam sandbox (`FETCH_MCP_SANDBOX=1`) the launcher grants that one variable and no other. Enforced in `httpRequest()`, the single path every tool takes to the network, from a policy each `createFetchServer()` call binds into its own tools -- servers in one process never share it. Before 0.7.1 the flag was honoured unconditionally; in 0.7.1 the setting was process-wide.
