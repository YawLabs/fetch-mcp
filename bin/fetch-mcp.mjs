 ...
 *
 * SELECTION
 *   FETCH_MCP_RUNTIME=oam    require oam; fail loudly if it is missing
 *   FETCH_MCP_RUNTIME=node   never use oam
 *   FETCH_MCP_RUNTIME=auto   prefer oam, silently fall back (default)
 *   FETCH_MCP_SANDBOX=1      run oam under --permission (oam 0.9.0+)
...
if (mode === "node") {
  ...
  if (!oam) {
    ...
    if (mode === "oam") {
      ...
      writeSync(
        2,
        "fetch-mcp: FETCH_MCP_RUNTIME=oam but no runnable oam binary was found.\n" + shimNote +
          "Install from https://oamjs.org, set OAM_BIN=/path/to/oam, or use FETCH_MCP_RUNTIME=node.\n",
      );
      process.exit(1);
    }
    // auto: falling back is correct, but silence is how someone never learns
    // their oam install is a shape this launcher skips.
    if (oamShim) await errSync(`fetch-mcp: ${shimNote}Using Node instead.\n`);
    await runInProcess();
    ...
    const remedy = found
      ? "Run \`oam self-update\`, or use FETCH_MCP_RUNTIME=node.\n"
      : "Check that it is an executable oam binary for this platform, or use FETCH_MCP_RUNTIME=node.\n";
    if (mode === "oam") {
      await errSync(`fetch-mcp: FETCH_MCP_RUNTIME=oam but ${detail}.\n${remedy}`);
      process.exit(1);
    }
    ...
    await errSync(`fetch-mcp: ${detail}; using Node instead.\n`);
    await runInProcess();
  } else {
    // `--` separates oam's own flags from the script's argv, so `fetch-mcp
    ...
    const launchFailed = async (err) => {
      if (mode === "oam") {
        await errSync(`fetch-mcp: failed to launch oam (${err?.message ?? err})\n`);
        process.exit(1);
      }
      await runInProcess();
    };
