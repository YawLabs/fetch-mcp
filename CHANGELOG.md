...
- **`fetch_sitemap` fetches at most `max_sitemaps` documents per call (default 50, max 1000).** An index listing N children produced N+1 requests, to as many hosts as it named, from one tool call. Children past the cap are listed under `childSitemaps`, unfetched, with a warning. The whole call also has one budget -- the same 5 minutes a single request gets -- that stops the fetch in flight and lists what is left, instead of up to `max_sitemaps` x `timeout_ms`.

### Changed
- **`timeout_ms` bounds a whole attempt -- DNS, every redirect hop and the body -- instead of each hop separately.** The DNS lookup now runs inside it: it used to run before the hop timer started, so a resolver that never answered added its full OS timeout to every hop. Retries get a fresh budget, and a whole call is capped at 5 minutes; at the schema maxima (6 attempts x 21 hops x 120 s, plus Retry-After waits) one call could run for hours. A slow redirect chain that fit when each hop had its own `timeout_ms` may now need a larger value. A retry starts only if its wait and a full attempt fit under the ceiling; otherwise the response already received is returned.
...
### Fixed
- **A request aborted while still connecting no longer holds the call for 10 s.** After a timeout or a cancellation during the TCP connect or TLS handshake, `sendHop()` awaited the pinned Agent's `close()`, which waits out undici's 10 s connect timeout -- `request exceeded 300ms` came back after ~10.6 s. A SYN that is never answered, or a server that accepts TCP and never completes TLS, was enough, on every guarded hostname request. The Agent is now destroyed rather than closed, and its connect timeout is what remains of the attempt's budget. Present since the IP-pinning refactor in 0.3.0.
...
### Security
- **SSRF: a redirect to a literal blocked IP, or to a non-http(s) scheme, is now refused. Every redirect hop runs the pre-flight URL validator.** `validateUrl()` -- scheme allow-list, literal-IP block list, `localhost*` -- ran on the initial URL only. A redirect `Location` reached the network through `sendHop()`, which re-validated hostnames (DNS resolve + IP pin) but skipped every check when the hop's host was a literal IP, on the false premise that literal IPs were "covered by validateUrl". With the guard on (`allow_private_hosts` unset), `http://1.2.3.4/` -> `302 Location: http://169.254.169.254/latest/meta-data/...` was fetched and its body returned; the same held for `http://127.0.0.1:8080/`, RFC 1918 targets, `http://[::1]/`, IPv4-mapped IPv6 (`[::ffff:169.254.169.254]`, `[::ffff:a9fe:a9fe]`), and the WHATWG-normalised spellings `http://2130706433/`, `http://0x7f000001/`, `http://127.1/`. The scheme allow-list was not re-checked per hop either, so a redirect to `ftp://1.2.3.4/` also reached `fetch`. `sendHop()` now runs `validateUrl()` on every URL it dials, so the per-hop re-validation SECURITY.md describes (defense 5) holds for literals and schemes as well as hostnames; hostname hops keep the DNS resolve + IP pin. Redirects to public literals, public hostnames and same-host relative paths follow as before, and `allow_private_hosts: true` still follows private redirects (the scheme allow-list holds there too). Affects 0.3.0 through 0.7.0: the literal skip arrived with the IP-pinning refactor in 0.3.0; 0.1.0 and 0.2.0 ran `resolveAndCheck()` on every hop, and `dns.lookup` hands a literal straight back, so `checkIpAddress` did reject it there. Reported privately by Arnav Garg ([@gargarnav](https://github.com/gargarnav)) on 2026-09-20; CWE-918. New suite `src/tests/http.redirect-ssrf.test.ts` pins the redirect cases -- 30 of its original 35 fail on the previous code.
- **`allow_private_hosts` now needs the operator's opt-in: `FETCH_MCP_ALLOW_PRIVATE_HOSTS=1`. Breaking for anyone who relied on the per-call flag alone.** The flag is a tool argument on all 15 tools, and the model chooses tool arguments -- so the prompt injection SECURITY.md's threat model describes could set `allow_private_hosts: true` next to `http://169.254.169.254/...` and the guard stepped aside. A call that sets the flag is now refused with `allow_private_hosts is disabled on this server: the operator has not set FETCH_MCP_ALLOW_PRIVATE_HOSTS=1 ...` before any DNS or network work, unless the server was launched with `FETCH_MCP_ALLOW_PRIVATE_HOSTS` set to `1`/`true`/`yes`/`on` (trimmed, case-insensitive). Any other value is treated as off and named on stderr -- the same strict, fail-closed grammar as `FETCH_MCP_SANDBOX`. With the variable set, a call still has to ask; calls without the flag stay guarded. To keep local-development fetches working, add `"env": { "FETCH_MCP_ALLOW_PRIVATE_HOSTS": "1" }` to the server's MCP config. The gate lives in `httpRequest()`, the one path every tool takes to the network, so no tool can skip it; every tool's `allow_private_hosts` description now tells the model about it. Under the oam sandbox (`FETCH_MCP_SANDBOX=1`) the launcher adds `--allow-env=FETCH_MCP_ALLOW_PRIVATE_HOSTS` -- without it the variable is invisible under `--permission` and the opt-in would silently read as off. Embedders: `createFetchServer()` refuses the flag by default; pass `createFetchServer({ allowPrivateHosts: true })` to allow it (`startServer()` reads the env var). The setting is process-wide -- the most recent `createFetchServer()` call decides for every server in the process; the stdio server creates exactly one.
- **`Cookie` and `Proxy-Authorization` are now stripped on cross-origin redirects, alongside `Authorization`.** Redirects are followed by hand, so undici's own stripping never runs, and `buildHeaders()` removed only `Authorization`: an `http_get` with `headers: { Cookie: "session=..." }` whose target 302'd to another origin handed the cookie to that origin. Arbitrary custom headers (`X-Api-Key`, ...) cannot be recognised by name and still follow; SECURITY.md now says so.
- **`fetch_sitemap` caps the DECOMPRESSED size of a gzipped sitemap at `max_bytes`, on Node and on oam.** `max_bytes` bounded only the compressed bytes read off the wire; `gunzipSync()` then inflated with no limit, so a ~200 KB `.xml.gz` expanded to ~200 MB inside the server (gzip reaches ~1000:1 on repetitive input) -- under oam that took the process down with `OAM-RT-OOM` at the 4 GB heap cap. The cap is counted by hand on a streaming gunzip (1 KiB input slices, destroyed once the output count passes the cap) rather than passed as zlib's `maxOutputLength`, because oam accepts that option and ignores it; measured, a 200 MiB bomb is refused in ~30 ms on both runtimes and the server keeps serving. The tool returns `parse failed: gzipped sitemap decompresses past N bytes (the max_bytes cap); raise max_bytes to read it`. `max_bytes` for the sitemap tool is also clamped to the 100 MiB ceiling `httpRequest()` already applied to the wire read, so a model-chosen `max_bytes` cannot lift the cap past it. Behaviour change for legitimate large sitemaps: a raw `.xml.gz` that inflates past 20 MiB now needs an explicit `max_bytes` (the same limit already applied to plain and `Content-Encoding: gzip` responses).

### Fixed
- **Guarded hostname requests work on plain Node again: every one of them failed with `fetch failed` on Node 22 and 24 from 0.4.0 through 0.7.0.** `src/http.ts` paired undici 8's `Agent` (the DNS-pinned dispatcher) with the runtime's global `fetch`. Node's bundled undici (6.x on 22, 7.x on 24) drives a dispatcher through the legacy handler API, and undici 8 rejects that with `UND_ERR_INVALID_ARG` ("invalid onRequestStart method"). So under Node -- `FETCH_MCP_RUNTIME=node`, or no oam installed -- only literal-IP URLs and `allow_private_hosts` calls got through; every `https://example.com/`-style request with the guard on failed. oam's `fetch` speaks the new API, which is why it went unnoticed. `fetch` now comes from the same `undici` package as the `Agent`. It broke in 0.4.0, when the `undici` floor moved from `^7.25.0` to `^8.5.0` (on Node 20 that same move stopped the server loading at all -- see the `engines` change below). New `src/tests/http.pinned-dispatch.test.ts` drives the real pinned dispatcher over a real socket; it fails with `fetch failed` if the global `fetch` comes back.
- **The launcher refuses to serve on a Node older than 22.19.0 with a clear message, instead of crashing.** undici 8's floor is Node 22.19.0. Below it the failure was never clean: Node 20 died at import (`TypeError: webidl.util.markAsUncloneable is not a function`) since 0.4.0, and Node 22 before 22.15 has no `zlib.createZstdDecompress`, so the first response that arrived with `content-encoding: zstd` (facebook.com, instagram.com and other Meta properties send it) threw inside undici's stream and took the whole server down mid-session. Every path that serves on Node -- in-process, the fallback, and the hand-off child, whose `node --version` is probed -- now exits 1 before serving with `fetch-mcp: <this process | path> is Node X.Y.Z, older than 22.19.0; @yawlabs/fetch-mcp needs Node 22.19.0 or newer ...` and names both remedies (a newer Node, or oam). The check runs before any "runs WITHOUT --permission" note, so a refusal never sits under a line that promised a server. oam hosts are exempt.

### Changed
- **`engines.node` is now `>=22.19.0` (was `>=20`).** It matches the runtime dependency the server has shipped since 0.4.0 (`undici` `^8.x`, whose own `engines` is `>=22.19.0`); `>=20` advertised a runtime on which the server could not start. Under oam the Node version does not matter. README and the package `author` field now point at `contact@yaw.sh` / yaw.sh.
- SECURITY.md: the reporting address is now `contact@yaw.sh` (`support@mcp.hosting` no longer delivers). It also documents the new defenses (operator gate, credential stripping, decompression cap) and two known limitations: custom credential headers and 307/308 bodies follow cross-origin redirects.
...
- **An oam host under the sandbox can now spawn a fresh oam even when none is on PATH or in an installed location.** Discovery used to consider only installed locations and PATH, so a host whose oam is bundled inside the app (Yaw MCP) reported "no usable oam was found" under the sandbox while being one, and dropped the sandbox (auto) or exited (oam). The running process's own binary is now the FIRST candidate, gated on its `--version` agreeing with `process.versions.oam` -- a wrapper or a Node posing as oam (the suite's own preload) cannot masquerade as one.

### Fixed
- **The `RUNTIME=oam` fatal no longer offers `RUNTIME=node` as a plain remedy when the sandbox was requested.** Plain advice would loop: the fallback note sends people to `RUNTIME=oam`, and this error would send them straight back. With the sandbox requested, the error now reads `or drop FETCH_MCP_SANDBOX=1 and use FETCH_MCP_RUNTIME=node (Node cannot apply the sandbox)`. Without the sandbox, the plain `RUNTIME=node` advice is still offered.
